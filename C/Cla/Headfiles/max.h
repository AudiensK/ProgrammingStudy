...
// 在 C 和 C++ 编程里，头文件保护宏（像_MAX_H_）采用大写形式是一种广泛使用的惯例
// 把预处理宏定义成大写，能让它们在代码里更加醒目，和普通变量、函数名形成鲜明对比。
// 采用全大写的方式，可以降低和小写的变量或函数名出现重复的可能性。
// _MAX_H_只是用于头文件的命名，用于条件判断，与头文件的文件名可以不一致，但是一般要保持一致增加辨识度
#ifndef _MAX_H_
#define _MAX_H_

#include <stdbool.h>  // 仅在头文件中包含必要的头文件，函数声明中用到了bool
...
// 头文件中仅包含声明，如果包含了定义，则违反了单一定义原则，可能会出现链接错误，代码膨胀等各种问题
